...
/** MiMo OpenAI 兼容 /chat/completions 单个 SSE chunk 的形状 */
interface MiMoChunk {
  choices?: {
...
/** 识别常见 MiMo 错误，返回可读中文提示；其他错误原样返回 */
function friendlyError(status: number, body: string): string {
  const b = body || "";
  if (b.includes("Invalid API Key") || b.includes("invalid api key") || b.includes("Unauthorized")) {
    return t("api.invalid_key");
  }
  if (b.includes("Insufficient Balance") || b.includes("insufficient user balance") || b.includes("余额")) {
    return t("api.insufficient_balance");
  }
  if (b.includes("Rate limit") || b.includes("rate limit")) {
    return t("api.rate_limit");
  }
  return `MiMo API ${status}: ${b.slice(0, 300)}`;
}

export class MiMoProvider implements LLMProvider {
  id = "mimo";
  /** 默认 base URL：OpenAI 兼容协议 */
  private defaultBase = "https://api.xiaomimimo.com/v1";

  async getModelMeta(model: AIModel): Promise<TestResult> {
    const base = model.baseUrl || this.defaultBase;
    // OpenAI 兼容网关：GET /models 校验模型名 + 回填限额；网关不实现时降级为联通性探测
  ...
  /** 最小联通性探测：POST /chat/completions，非流式、极短消息 */
  ...
  async chat(
    ...
    // 与 Gemini / DeepSeek 一致：流式优先 XHR（SSE），失败且未输出时 requestUrl 兜底
    ...
    const handleDataJson = (json: string): void => {
      try {
        const data = JSON.parse(json) as MiMoChunk;
        const piece =
        ...
        // stream_options.include_usage 下，最后一块 chunk 带 usage
    ...
    // 部分网关不支持 stream_options，报 400 时去掉重发（不重复输出）
    const send = async (
